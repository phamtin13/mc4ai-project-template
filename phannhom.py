...
def phannhom():
  ...
  kmeans.fit(X)
  st.write(px.scatter_3d(dfmid, x = 'Homework', y = 'Midterm Exam', z = 'Final Exam', color = kmeans.labels_).update_layout(showlegend=True))
  ...
  for j in labels:
    ...
    new = df_new['GPA']
    st.subheader(j+':')
    st.write('Số học sinh trong nhóm:',len(df_need),'.')
