...
def danhsach():
  ...
  with col1:
    ...
    check_gender = [str(st.checkbox(i,value=True,key=unigen[i]+' key')) for i in unigen]
  ...
  for i in range(2):
    for j in range(5):
      check_sub.append(str(cols[j].checkbox(unisub[5*i+j],value=True,key=unisub[5*i+j]+' key')))
